...
export const Route = createFileRoute("/my-matches")({
  ...
  head: () => ({
    title: "Your Scheme Matches – Yojantra",
      ...
      try {
        ...
        const annotated = (rows || []).map((r) => {
          try {
            ...
            const fieldEvaluations: Record<
              string,
              { status: "MATCH" | "MISMATCH" | "UNKNOWN"; messages: string[] }
            > = {};
            const pushField = (
              tag: string,
              ...
              else if (status === "UNKNOWN" && fieldEvaluations[tag].status !== "MISMATCH")
                fieldEvaluations[tag].status = "UNKNOWN";
            };
        ...
        <div className="mt-4 flex items-center justify-between gap-4">
          <div className="relative w-full max-w-md">
            <input
              ...
              className="w-full rounded-full border border-ink/10 py-3 px-4 text-sm"
            />
            <Search className="absolute right-3 top-3 size-4 text-ink/40" />
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => void load(true)}
              className="inline-flex items-center gap-2 rounded-full border border-ink/15 px-4 py-2 text-[10px] font-semibold tracking-[.14em] uppercase"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-ink px-4 py-2 text-[10px] font-semibold tracking-[.14em] text-ivory uppercase"
            >
              Maximize benefits
            </button>
          </div>
        ...
        {showMaximizer && (
          <div className="mt-8 bg-white p-6">
            ...
            <p className="mt-2 text-sm text-ink/60">
              Grouped opportunities that may be potentially useful together. This does not guarantee
              combinability — check the official scheme rules before applying.
            </p>
        ...
        {!loading && !generating && !error && recommendations.length === 0 && (
          <div className="mt-12 bg-white p-10 text-center">
            <p className="text-sm leading-relaxed text-ink/60">
              We couldn't confirm strong matches yet.
            </p>
            <Link
              to="/personalize"
        ...
        {showDetails && (
          <div className="mt-3 rounded bg-ivory p-3 text-sm text-ink/70">
            {item.fieldEvaluations && Object.keys(item.fieldEvaluations).length > 0 && (
              <div className="mb-3">
                <div className="mb-2">
                  <strong>Eligibility by field</strong>
                </div>
                <ul className="mb-3 space-y-2">
                  {Object.entries(item.fieldEvaluations).map(([field, info]: any) => (
                    <li key={field} className="flex items-start gap-3">
                      {info.status === "MATCH" ? (
                        <Check className="size-4 text-verified mt-1" />
                      ) : info.status === "MISMATCH" ? (
                        <X className="size-4 text-ink/50 mt-1" />
                      ) : (
                        <AlertTriangle className="size-4 text-saffron mt-1" />
                      )}
                      <div>
