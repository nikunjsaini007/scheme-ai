  ...
  return (
    <main className="overflow-x-hidden bg-ivory text-ink">
        ...
        <div className="mt-10 grid gap-12 lg:grid-cols-12">
          <div className="lg:col-span-7">
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
              ...
              <span className="eyebrow text-ink/40">{scheme.category}</span>
            </div>
            <LineReveal
              as="h1"
              className="display mt-6 text-[14vw] leading-[0.84] lg:text-[7vw]"
              lines={scheme.name.split(" ").map((w) => w)}
            />
            <Reveal delay={0.2}>
              <p className="mt-8 max-w-md text-base text-ink/65">{scheme.summary}</p>
            </Reveal>
          </div>
          ...
          <Reveal delay={0.15} className="lg:col-span-5">
            <div className="bg-ink p-8 text-ivory md:p-10">
              ...
              <p className="eyebrow mt-4 text-ivory/45">{scheme.benefitNote}</p>
              {scheme.deadlineDays && (
                <div className="mt-8 flex items-baseline justify-between border-t border-ivory/15 pt-6">
                  <span className="eyebrow text-ivory/45">Deadline</span>
                  <span className="display text-3xl">{scheme.deadlineDays} days</span>
                </div>
              )}
              <a
                href="#apply"
      ...
      <section className="edge grid gap-14 py-16 lg:grid-cols-2 lg:py-24">
        {[
          ...
          <Reveal key={block.title}>
            <h2 className="display text-4xl md:text-5xl">{block.title}</h2>
      ...
      <section className="bg-ivory-deep py-24">
        <div className="edge">
          <LineReveal className="display text-[12vw] leading-[0.88] md:text-[6vw]" lines={["What you'll need"]} />
            ...
            {scheme.documents.map((d) => (
              <div key={d.name} className="border-b border-ink/12">
                ...
                >
                  ...
                  >
                    {d.mandatory ? "Mandatory" : "If applicable"}
                  </span>
                  <ChevronDown
                  ...
                  {open === d.name && (
                    ...
                    >
                      <div className="grid gap-6 pb-8 md:grid-cols-2 md:pl-8">
                        <div>
                          <p className="eyebrow text-ink/40">Why it's required</p>
                          <p className="mt-2 text-sm text-ink/75">{d.why}</p>
                        </div>
                        <div>
                          <p className="eyebrow text-ink/40">How to obtain it</p>
                          <p className="mt-2 text-sm text-ink/75">{d.how}</p>
                        </div>
                      </div>
                    </motion.div>
                  )}
      ...
      <section id="apply" className="edge py-24 md:py-32">
        <div className="grid gap-14 lg:grid-cols-12">
          <div className="lg:col-span-7">
            <LineReveal className="display text-[12vw] leading-[0.88] md:text-[5.4vw]" lines={["How to apply"]} />
              ...
              {scheme.howToApply.map((s, i) => (
                <Reveal key={s} delay={i * 0.06}>
                  <li className="flex gap-6 border-b border-ink/12 py-6">
                    ...
                    <p className="pt-1 text-sm text-ink/75 md:text-base">{s}</p>
                  </li>
                </Reveal>
              ))}
            </ol>
          ...
          <Reveal className="lg:col-span-4 lg:col-start-9">
            <div className="bg-white p-8">
              <p className="eyebrow text-ink/40">Official source</p>
              ...
              >
                Go to official portal <ExternalLink className="size-3.5" />
              </a>
              ...
              <p className="mt-6 text-[11px] leading-relaxed text-ink/45">
                Demo data. Applications are submitted only on the official government portal — Yojantra never
                collects fees or processes applications.
              </p>
            </div>
          </Reveal>
        </div>
