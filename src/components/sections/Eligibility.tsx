    ...
    <section id="eligibility" className="bg-ink py-28 text-ivory md:py-40">
      <div className="edge">
        <Reveal className="mb-10 flex items-baseline gap-4 border-t border-ivory/15 pt-4">
          ...
          <span className="eyebrow text-ivory/40">Eligibility</span>
        </Reveal>

        <div className="grid gap-14 lg:grid-cols-12">
          <div className="lg:col-span-4">
            <LineReveal className="display text-[14vw] leading-[0.85] lg:text-[6vw]" lines={["Let's see", "what fits."]} />
            <Reveal delay={0.2}>
            ...
            <AnimatePresence mode="wait">
              {!done ? (
                <motion.div
                  ...
                  className="pt-10"
                >
                  <p className="display text-4xl md:text-6xl">{q.key}</p>
                  ...
                  <p className="mt-10 text-[11px] tracking-wide text-ivory/30">
                    Also asked in the full flow: gender, and any special conditions such as disability or
                ...
                >
                  ...
                  <dl className="mt-12 grid grid-cols-2 gap-x-8 gap-y-3 border-t border-ivory/15 pt-6 sm:grid-cols-3">
                    {QUESTIONS.map((qq) => (
                      <div key={qq.key}>
                        <dt className="eyebrow text-ivory/35">{qq.key}</dt>
                        <dd className="mt-1 text-sm text-ivory/85">{answers[qq.key] ?? "—"}</dd>
                      </div>
                    ))}
                  </dl>
                </motion.div>
