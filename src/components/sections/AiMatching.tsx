...
export function AiMatching() {
  ...
  return (
    <section id="matching" ref={ref} className="edge overflow-hidden py-28 text-center md:py-44">
      <LineReveal className="display text-[14vw] leading-[0.86] md:text-[7.5vw]" lines={["Reading", "your profile."]} />
      ...
      <LineReveal className="display mt-10 text-[12vw] leading-none text-saffron md:text-[6vw]" lines={["Your matches"]} />
        ...
        {STATS.map((s, i) => (
          <Reveal key={s.label} delay={i * 0.08} className="bg-ivory">
            <div className="p-6 text-left md:p-8">
              ...
              <p className="mt-3 text-[11px] tracking-[0.14em] text-ink/50 uppercase">{s.label}</p>
            </div>
          </Reveal>
        ))}
      </div>
