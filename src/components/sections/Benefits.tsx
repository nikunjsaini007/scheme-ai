...
export function Benefits() {
  return (
    <section id="benefits" className="bg-ivory-deep py-28 md:py-40">
      <div className="edge">
        <SectionLabel index="04" title="Benefit visualization" />
        <LineReveal className="display text-[14vw] leading-[0.85] md:text-[8vw]" lines={["What could", "you get?"]} />
          ...
          {FIGURES.map((f, i) => (
            <Reveal key={f.label} delay={i * 0.1}>
              <div className={i % 2 ? "lg:mt-10" : ""}>
                ...
                />
                <p className="mt-4 max-w-[14ch] text-[11px] tracking-[0.14em] text-ink/50 uppercase">{f.label}</p>
              </div>
            </Reveal>
          ))}
        </div>
        ...
        <Reveal>
          <p className="mt-20 max-w-lg border-t border-ink/15 pt-6 text-[12px] leading-relaxed text-ink/45">
            Estimated potential benefits based on matching schemes. These are indicative figures from mock data —
            not a guaranteed government payout, and not an approval of any application.
          </p>
        </Reveal>
      </div>
    </section>
