...
export function Results() {
  ...
  return (
    <section id="results" className="edge py-28 md:py-40">
      ...
      <LineReveal className="display text-[15vw] leading-[0.85] md:text-[8vw]" lines={["These", "are for you."]} />
      <Reveal delay={0.2}>
        ...
        >
          <div className="flex items-center justify-between">
            ...
            {feature.deadlineDays && (
              <span className="eyebrow text-ivory/45">Deadline · {feature.deadlineDays} days</span>
            )}
          </div>
          <p className="display mt-10 text-[22vw] leading-[0.8] text-saffron md:text-[9vw]">{feature.benefit}</p>
          <p className="eyebrow mt-4 text-ivory/45">{feature.benefitNote.replace("Maximum", "Potential")}</p>
          <h3 className="display mt-10 text-4xl md:text-6xl">{feature.name}</h3>
          <p className="mt-4 max-w-sm text-sm text-ivory/65">{feature.summary}</p>
          <div className="mt-10 flex flex-wrap items-center gap-x-8 gap-y-3 border-t border-ivory/15 pt-6">
            <span className="eyebrow text-ivory/45">{feature.category}</span>
            <span className="eyebrow text-ivory/45">{feature.level}</span>
            <Link
              to="/scheme/$id"
          ...
          >
            ...
            <div className="flex flex-col justify-between p-7 md:col-span-7 md:p-12">
              <div>
                ...
                <h3 className="display mt-4 text-5xl md:text-7xl">{hero3.name}</h3>
                <p className="mt-5 max-w-md text-sm text-ink/60">{hero3.summary}</p>
              </div>
              <div className="mt-10 flex flex-wrap items-baseline gap-8">
                <div>
                  <p className="display text-5xl text-saffron">{hero3.benefit}</p>
                  <p className="eyebrow mt-2 text-ink/45">{hero3.benefitNote}</p>
                </div>
                <span className="eyebrow ml-auto text-ink/45">{hero3.category}</span>
          ...
          >
            ...
            >
              <div>
                ...
                <h3 className="display mt-4 text-3xl leading-[0.92] md:text-4xl">{s.name}</h3>
                <p className="mt-4 text-sm opacity-65">{s.summary}</p>
              </div>
              <div className="mt-10">
                <p className="display text-5xl">{s.benefit}</p>
                <p className="mt-2 text-[10px] tracking-[0.16em] uppercase opacity-55">{s.benefitNote}</p>
